...
const FAQ = () => {
  ...
  const fetchFaqs = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get("http://141.11.25.167:8080/api/faqs");
  ...
  const saveEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://141.11.25.167:8080/api/faqs/${currentFaq.id}`,
        {
  ...
  const saveNewFaq = async () => {
    try {
      const response = await axios.post("http://141.11.25.167:8080/api/faqs", {
  ...
  const confirmDelete = async (faq: FAQItem) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://141.11.25.167:8080/api/faqs/${faq.id}`);
